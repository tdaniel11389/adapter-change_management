...
function processRequestResults(error, response, body, callback) {
   ...
   */
   ...
   if (error) {
       ...
       console.log('repsonse code error');
       callbackErrors=response;
    
   }
   else if (isHibernating(response)){
       console.log(callbackErrors);
       callbackErrors='the instance is hibernating';
   }
   else {
       callbackData=response; 
   }
   return callback(callbackData, callbackErrors)
}
